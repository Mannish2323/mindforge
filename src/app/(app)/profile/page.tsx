  ...
  return (
    <div className="max-w-[1400px] mx-auto animate-fade-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8 items-start">
        ...
        {/* COLUMN 1: Identity & Level Progress */}
        <div className="space-y-4 md:space-y-6 lg:col-span-1">
            ...
            <div className="px-4 sm:px-5 pb-5">
              ...
              <div className="flex flex-wrap items-end gap-3 -mt-8 sm:-mt-10 mb-3">
                ...
                <div className="relative flex-shrink-0">
                  <div
                    className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl border-[3px] sm:border-4 overflow-hidden relative animate-float"
                    style={{ borderColor: '#0e0b22', background: 'linear-gradient(135deg, #7c3aed, #db2777)', animationDuration: '4s' }}
                  >
                    {profile?.avatarUrl ? (
              ...
              <div className="mb-2 min-w-0">
                {editingName ? (
                  ...
                  <div className="flex items-center gap-2 min-w-0 overflow-hidden">
                    <h2 className="text-xl sm:text-2xl font-black text-white truncate min-w-0 max-w-[calc(100%-2.5rem)]">
          ...
          {/* Logout (Desktop only, hidden on mobile) */}
          <button
            onClick={() => logout()}
            className="hidden md:flex w-full items-center justify-center gap-2 h-14 rounded-xl text-sm font-bold transition-all"
        ...
        {/* RIGHT AREA: Spans 2 columns on desktop, stacked on mobile */}
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 items-start">
          
          {/* CENTER PANEL (Stats & Achievements) */}
          <div className="space-y-4 md:space-y-6">
            {/* ── Stats Grid ────────────────────────── */}
            <div className="grid grid-cols-2 gap-3">
              {stats.map(s => (
                <Card key={s.label} padding="sm" className="sm:p-4 hover:scale-[1.02] transition-transform">
          ...
          {/* RIGHT PANEL (Subscription & Quick Links) */}
          <div className="space-y-4 md:space-y-6">
            {/* ── Subscription Card ─────────────────── */}
            <SubscriptionCard />
      ...
      {/* Logout button (Mobile only, hidden on tablet/desktop) */}
      <button
        onClick={() => logout()}
        className="flex md:hidden w-full items-center justify-center gap-2 h-14 rounded-xl text-sm font-bold transition-all mt-4"
        style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.15)', color: '#f87171' }}
      >
        <LogOut className="w-4 h-4" />
        Sign Out
