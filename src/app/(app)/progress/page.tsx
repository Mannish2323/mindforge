  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      <motion.div variants={item} className="space-y-1">
        ...
        <p className="text-sm text-purple-300/45">Track your Japanese learning journey</p>
      </motion.div>
      ...
      <motion.div variants={item} className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Level', value: `Lv.${level}`, icon: TrendingUp, color: 'text-neon-pink' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div variants={item}>
          <Card variant="glass" padding="md" className="flex flex-col items-center gap-4">
            <h3 className="text-[10px] font-extrabold text-purple-300/30 uppercase tracking-[0.2em] self-start">
              JLPT Readiness
            </h3>
            <ProgressRing value={35} size={130} strokeWidth={10} label={profile?.jlpt_target || 'N5'} />
            <p className="text-xs text-purple-300/40">Estimated readiness for {profile?.jlpt_target || 'N5'}</p>
          </Card>
        </motion.div>
      </div>
      ...
      <motion.div variants={item}>
        <Card variant="glass" padding="md" className="space-y-4">
          <h3 className="text-[10px] font-extrabold text-purple-300/30 uppercase tracking-[0.2em]">Skill Breakdown</h3>
      ...
      <motion.div variants={item} className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <Card variant="glass" padding="md" className="text-center">
          <span className="text-2xl font-bold text-white font-orbitron block">{wordsLearned}</span>
          <span className="text-[10px] text-purple-300/40 font-bold uppercase">Words Learned</span>
        </Card>
        <Card variant="glass" padding="md" className="text-center">
          <span className="text-2xl font-bold text-white font-orbitron block">{kanjiLearned}</span>
          <span className="text-[10px] text-purple-300/40 font-bold uppercase">Kanji Learned</span>
        </Card>
        <Card variant="glass" padding="md" className="text-center col-span-2 md:col-span-1">
          <span className="text-2xl font-bold text-white font-orbitron block">{profile?.reviews_done ?? 0}</span>
          <span className="text-[10px] text-purple-300/40 font-bold uppercase">Reviews Done</span>
        </Card>
      </motion.div>
    </motion.div>
  );
