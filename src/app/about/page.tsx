    ...
    >
      ...
      <header className="w-full border-b border-white/[0.08] bg-[#09070F]/80 backdrop-blur-xl z-20 sticky top-0">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/home" className="flex items-center gap-2 text-xs font-bold text-purple-300/60 hover:text-white transition-colors cursor-pointer group">
            <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
            <span>Back to Home</span>
          </Link>
          
          <div className="flex items-center gap-2 text-xs font-bold text-purple-300/40 uppercase tracking-widest font-orbitron">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 z-10 space-y-12">
        ...
        <div className="text-center space-y-4">
          <span className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-neon-purple/10 border border-neon-purple/20 text-xs font-bold text-brand-light uppercase tracking-wider">
            <Sparkles className="w-3.5 h-3.5" />
            Yample Labs Software Project
          </span>
          <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight font-orbitron">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {stats.map((s, idx) => (
            <div key={idx} className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] text-center space-y-1">
              <span className="text-2xl font-extrabold text-white font-orbitron">{s.value}</span>
              <p className="text-[11px] text-purple-300/40 font-bold uppercase tracking-wider">{s.label}</p>
            </div>
          ))}
        </div>

        {/* Company & Founder Section */}
        <div className="p-8 rounded-3xl bg-[#12101D] border border-white/[0.08] space-y-6">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-neon-purple/10 border border-neon-purple/20 text-brand-light">
              <Building2 className="w-6 h-6" />
            </div>
            <div>
              <h2 className="text-xl font-extrabold text-white font-orbitron">Yample Labs</h2>
              <p className="text-xs text-purple-300/50">Parent Company & Engineering Organization</p>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-white/[0.06] flex items-center justify-between text-xs text-purple-300/40 font-medium">
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-extrabold text-white font-orbitron text-center sm:text-left">
            Core Pillars
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {values.map((v, idx) => (
              <div key={idx} className="p-6 rounded-2xl bg-white/[0.02] border border-white/[0.06] space-y-3">
                <div className="p-3 w-fit rounded-xl bg-neon-purple/10 border border-neon-purple/20 text-brand-light">
                  <v.icon className="w-5 h-5" />
                </div>
                <h3 className="text-base font-bold text-white">{v.title}</h3>
                <p className="text-xs text-purple-300/60 leading-relaxed">{v.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Footer */}
      <footer className="w-full border-t border-white/[0.06] py-6 text-center text-[10px] text-purple-300/20 font-bold uppercase tracking-widest px-6 z-20">
